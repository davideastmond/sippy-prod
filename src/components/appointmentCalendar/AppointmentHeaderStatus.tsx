...
export function HeaderStatus({ stages, currentStage }: StageProgressProps) {
  return (
    <>  
      <TextLabel 
        text='We need some information to create your profile! Oh, and you can edit this information later.'
        color='Gray-600'
        fontSize='Text-20'
      />
            ...
            <div key={stage} className="flex items-center">
              <div
                className={`h-1.5 w-full rounded-full flex items-center justify-center ${
                  index <= currentStage ? 'bg-simmpy-gray-100' : 'bg-simmpy-gray-600'
                }`}
              />
            </div>
          ))}
